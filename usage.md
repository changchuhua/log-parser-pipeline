...
| `noise_max_retries` | `2` | Re-queue attempts for DBSCAN noise logs before falling back to regex pre-masking. Only relevant under `noise_mode: "production"`. |
| `noise_mode` | `"production"` | `"production"` (default): our own 3-tier fallback — cache match → re-queue (`noise_max_retries`) → regex pre-mask, no LLM call, audited via `quarantine.jsonl`. `"original"`: a faithful port of upstream LogBatcher's `reassign_clusters()` (`additional_cluster.py`'s `_reassign_noise_labels`/`_tokenize_for_noise_grouping`) — groups DBSCAN noise logs with an *exactly identical tokenized string* into new clusters, promotes every other noise log to its own new cluster, all inside `SimilarityCluster.get_partitions()` itself, before any LLM work. Real behavioral tradeoff, not just a different algorithm: reassigned clusters flow through the normal sample/LLM/cache path like any other cluster, so they **can** cost an LLM call and get **no** `quarantine.jsonl` entry — they were never quarantined. Only affects `cluster: "SimilarityCluster"`; `LengthCluster` has no noise concept and ignores this key harmlessly. See `parser_implementation_comparison.md` Section 1 for the full comparison. |
...
| `cache_mode` | `"production"` | `"production"` (default): our own hardened cache — `OrderedDict`-based LRU (`max_size=5000`), medoid-level Jaccard reconciliation, anchored-regex verification (`match_and_prune`). `"original"`: a faithful port of upstream LogBatcher's actual cache (`core/logbatcher/original_cache.py`) — SHA256 hash-exact-match fast path, prefix-tree structural matching, per-log (not medoid-level) reconciliation. See `parser_implementation_comparison.md` Section 1 for the full comparison, including two disclosed limitations of the `"original"` mode: `--use-cache` can't meaningfully warm-start the hash fast path (it's keyed by hashes of specific historical logs, not persisted), and the ported LCS-merge-on-insert capability is deliberately never triggered, matching upstream's own real call pattern exactly. |
| `postprocess_mode` | `"production"` | `"production"` (default): `clean_template()` strips markdown/backticks only. `"original"`: additionally runs upstream's full `correct_single_template()` normalization cascade (path-string collapsing, boolean/default-string substitution, digit-heavy token detection, word+variable merging, consecutive/dot-separated `<*>` collapsing, size-unit collapsing) on fresh LLM output, gated by upstream's `verify_template()` degenerate-output check — a template that's nothing but `<*>`/punctuation falls back to a rule-only pass on the raw log, no LLM. |
| `prompt_mode` | `"production"` | `"production"` (default): our own worked-example prompt, direct `<*>` output. `"original"`: a faithful port of upstream's actual zero-shot prompt — a 15-type variable taxonomy (url, IPv4_port, path, date, size, user_name, etc.), historical-variables grounding (a growing pool of previously-extracted values fed back as few-shot-style hints), and `{{placeholder}}`/`${...}` output convention, converted back to `<*>` before the result is returned — so callers get a `<*>`-normalized template either way, independent of `postprocess_mode`. Includes a disclosed, non-upstream anti-preamble instruction (see doc note) so local models don't wrap the answer in conversational explanation. |
| `historical_variables_cap` | `100` | Only applies under `prompt_mode: "original"`. Upstream includes its **entire, unbounded** `variable_candidates` list in every prompt. Benchmarked directly against real logs: LLM latency scales ~linearly with list size (1.0s @ 0 vars → 15.2s @ 1000 vars), so over a long run cumulative time grows roughly quadratically with call count — fine at the paper's original benchmark scale, not at a full-`loghub` (1.17M log) scale. This caps the prompt to the most recent N entries instead. `null` = uncapped, exact upstream fidelity. |
| `similar_sampler_mode` | `"production"` | `"production"` (default): medoid + Jaccard nearest-neighbor selection. `"original"`: upstream's actual "similar" sample method — K-means clustering on TF-IDF vectors of the message text (`group_samples_clustering()`), no embedding model needed either way. Only affects `sampler: "SimilarSampler"`. |
| `dpp_kernel_mode` | `"production"` | `"production"` (default): DPP selection over an LLM-embedding similarity kernel, with `embedding_length_threshold`/Jaccard fallback for oversized or failed embeddings. `"original"`: upstream's actual kernel source (`Cluster.sample()` in `cluster.py`) — TF-IDF vectors of the log text, cosine similarity, no embedding calls and no fallback needed (TF-IDF has no failure mode analogous to an embedding API call). The greedy selection algorithm itself (`_greedy_dpp_select`) is identical either way — only the kernel input differs. Only affects `sampler: "DPPSampler"`. |
| `batch_truncation_mode` | `"production"` | `"production"` (default): sends the sampler's full output to the LLM. `"original"`: upstream's `not_varibility()` check — if the sampled batch is identical once digits are stripped (no real diversity beyond numbers), truncate to 3 logs before querying, to avoid spending LLM budget on a degenerate batch. |
